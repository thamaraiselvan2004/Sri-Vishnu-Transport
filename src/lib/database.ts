...
export async function addTrip(
  ...
  const client = requireSupabase();
  const payload = tripPayload(tripData);
...
export async function getMaintenanceRecords(
  ...
  return ((data ?? []) as MaintenanceRecord[]).map((record) => ({
    ...record,
    vehicle_number: vehicleMap.get(record.vehicle_id) ?? record.vehicle_number,
  }));
}
...
export async function addMaintenanceRecord(
  ...
  const { data, error } = await requireSupabase()
    .from("maintenance")
    .insert(maintenancePayload(record))
    .select()
    .single();
  if (error) throw error;
  ...
  return {
    ...
    vehicle_number: vehicles.find((v) => v.id === data.vehicle_id)?.vehicle_number,
  };
}
...
export async function clearAllTripsAndMaintenance(): Promise<void> {
  const client = requireSupabase();
  const { error: tripsError } = await client
    .from("trips")
    .delete()
    .not("id", "is", null);
  if (tripsError) throw tripsError;

  const { error: maintenanceError } = await client
    .from("maintenance")
    .delete()
    .not("id", "is", null);
