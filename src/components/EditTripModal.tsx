...
export const EditTripModal: React.FC<EditTripModalProps> = ({
  ...
  useEffect(() => {
    ...
    setAmountPaidToDriver(String(trip.amount_paid_to_driver ?? ""));
  ...
  const driverBeta = calculateDriverBeta(
    ...
    driverBetaType,
    numberValue(manualDriverBeta)
  );
  const balanceAmount =
    numberValue(tripFare) - numberValue(brokerFare) - numberValue(advanceReceived) + haltingFare;
  const remainingDriverAmount = driverBeta - numberValue(amountPaidToDriver);
  const netProfit = calculateTripNetProfit({
    tripFare: numberValue(tripFare),
    haltingFare,
    brokerFare: numberValue(brokerFare),
    driverBeta,
    loadingExpense: numberValue(loadingExpense),
    unloadingExpense: numberValue(unloadingExpense),
    tollCharges: numberValue(tollCharges),
    dieselExpense: numberValue(dieselExpense),
    otherExpenses: numberValue(otherExpenses),
  });
  ...
  const save = async () => {
    if (!trip) return;
    if (!vehicleId || !driverId || !tripDate || !tripFare) {
      setError("Please fill Vehicle, Driver, Trip Date and Trip Fare.");
      return;
    }
    setIsSaving(true);
    setError("");
    try {
      const updatedTrip: Trip = {
        ...
        diesel_expense: numberValue(dieselExpense),
        mileage: mileage.mileage,
        trip_fare: numberValue(tripFare),
        broker_fare: numberValue(brokerFare),
        driver_beta: driverBeta,
        driver_beta_type: driverBetaType,
        toll_charges: numberValue(tollCharges),
        loading_expense: numberValue(loadingExpense),
        unloading_expense: numberValue(unloadingExpense),
        other_expenses: numberValue(otherExpenses),
        ...
        amount_paid_to_driver: numberValue(amountPaidToDriver),
        driver_payment_date: driverPaymentDate || undefined,
        remaining_amount_to_driver: remainingDriverAmount,
        net_profit: netProfit,
        vehicle_number: selectedVehicle?.vehicle_number || trip.vehicle_number,
        driver_name: selectedDriver?.driver_name || trip.driver_name,
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/70 p-4 overflow-y-auto">
      <div className="my-6 w-full max-w-4xl overflow-hidden rounded-3xl bg-white shadow-2xl">
        <div className="flex items-center justify-between bg-slate-900 px-6 py-5 text-white">
          <div>
            <h2 className="text-lg font-bold">Edit Trip</h2>
            <p className="text-xs text-slate-400">Update trip, expense and payment details</p>
          </div>
        ...
        <div className="max-h-[78vh] overflow-y-auto p-6 space-y-6">
          ...
          <section>
            <h3 className="mb-3 text-sm font-bold uppercase tracking-wide text-slate-500">Trip Details</h3>
          ...
          <section>
            <h3 className="mb-3 text-sm font-bold uppercase tracking-wide text-slate-500">Distance & Fuel</h3>
            ...
            <div className="mt-3 rounded-xl bg-slate-50 p-3 text-sm text-slate-700">Mileage: <b>{mileage.mileage ? `${mileage.mileage} km/L` : "—"}</b></div>
          </section>

          <section>
            <h3 className="mb-3 text-sm font-bold uppercase tracking-wide text-slate-500">Fare & Driver Beta</h3>
          ...
          <section>
            <h3 className="mb-3 text-sm font-bold uppercase tracking-wide text-slate-500">Other Expenses</h3>
            <div className="grid grid-cols-1 gap-4 md:grid-cols-4">
          ...
          <section>
            <h3 className="mb-3 text-sm font-bold uppercase tracking-wide text-slate-500">Collections & Driver Settlement</h3>
            <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
        ...
        <div className="flex items-center justify-end gap-3 border-t border-slate-200 bg-slate-50 px-6 py-4">
          <button onClick={onClose} className="rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-sm font-semibold text-slate-700">Cancel</button>
          <button onClick={save} disabled={isSaving} className="inline-flex items-center gap-2 rounded-xl bg-blue-600 px-5 py-2.5 text-sm font-semibold text-white disabled:opacity-50">
